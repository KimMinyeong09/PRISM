...
class PrismScoreModel {
  ...
  PrismScoreModel(
      ...
      required this.Srank,
      required this.Grank,
      required this.indOverallRank,
      required this.indERank,
      required this.indSRank,
      required this.indGRank,
      required this.WOverallScore,
      required this.WEScore,
      required this.WSScore,
      required this.WGScore,
      required this.WOverallRank,
      required this.WERank,
      required this.WSRank,
      required this.WGRank,
      required this.companyId,
      required this.prismIndAvgId,
      required this.indWeightId,
      required this.kcgsScoreId,
      required this.esglabScoreId});
}
...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  final List<String> gri_subs_name = [
    "직접적 경제가치 발생과 분배(EVG&D)",
    "기후변화에 따른 재무적 영향 및 기타 리스크와 기회",
    "확정급여형 연금 채무 및 기타 퇴직연금안",
    "정부 재정지원",
  ...
  void fetchData() async {
    try {
      companyYears = await ApiService.outCompanyYears('AJ네트웍스');
    ...
    return List.generate(comparing_items.length, (index) {
      ...
      return Row(
        ...
        children: [
          ...
          Row(
            children: [
              _buildDChart(companyName, "PRISM 스코어", prism_scores[index],
                  prism_ind_avg_scores[index]),
              _buildDChart(companyName, "E", prism_scores[index],
                  prism_ind_avg_scores[index]),
              _buildDChart(companyName, "S", prism_scores[index],
                  prism_ind_avg_scores[index]),
              _buildDChart(companyName, "G", prism_scores[index],
                  prism_ind_avg_scores[index]),
            ],
          ),
          // wPRISM 스코어
          Row(
            children: [
              _buildDChart(companyName, "wPRISM 스코어", prism_scores[index],
                  prism_ind_avg_scores[index]),
              _buildDChart(companyName, "wE", prism_scores[index],
                  prism_ind_avg_scores[index]),
              _buildDChart(companyName, "wS", prism_scores[index],
                  prism_ind_avg_scores[index]),
              _buildDChart(companyName, "wG", prism_scores[index],
                  prism_ind_avg_scores[index]),
            ],
          ),
          // KCGS
          ...
          if (comparing_gris.isEmpty)
            SizedBox(
                height: MediaQuery.of(context).size.height / 4,
                child: const Text("GRI index를 선택해주세요 (최대 5개)"))
          else
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          ...
                          Text(
                            gri_subs.contains(comparing_gri)
                                ? gri_subs_name[gri_subs.indexOf(comparing_gri)]
                                : "",
                            style: const TextStyle(fontWeight: FontWeight.bold),
                          ),
                        ],
  ...
  Column _buildDChart(String companyName, String title,
      PrismScoreModel prismScore, PrismIndAvgScoreModel prismIndAvgScore) {
    int score = -1, indScore = -1, rank = -1, indRank = -1;
    switch (title) {
      case "PRISM 스코어":
        score = prismScore.overallScore;
        indScore = prismIndAvgScore.overallScore;
        rank = prismScore.overallRank;
        indRank = prismScore.indOverallRank;
      case "E":
        score = prismScore.EScore;
        indScore = prismIndAvgScore.EScore;
        rank = prismScore.Erank;
        indRank = prismScore.indERank;
      case "S":
        score = prismScore.SScore;
        indScore = prismIndAvgScore.SScore;
        rank = prismScore.Srank;
        indRank = prismScore.indSRank;
      case "G":
        score = prismScore.GScore;
        indScore = prismIndAvgScore.GScore;
        rank = prismScore.Grank;
        indRank = prismScore.indGRank;
      case "wPRISM 스코어":
        score = prismScore.WOverallScore;
        indScore = prismIndAvgScore.wOverallScore;
        rank = prismScore.WOverallRank;
        indRank = prismScore.indWeightId;
      case "wE":
        score = prismScore.WEScore;
        indScore = prismIndAvgScore.wEScore;
        rank = prismScore.Erank;
        indRank = prismScore.indERank;
      case "wS":
        score = prismScore.WSScore;
        indScore = prismIndAvgScore.wSScore;
        rank = prismScore.Srank;
        indRank = prismScore.indSRank;
      case "wG":
        score = prismScore.WGScore;
        indScore = prismIndAvgScore.wGScore;
        rank = prismScore.Grank;
        indRank = prismScore.indGRank;
    }
                  ...
                  if (rowIndex >= one_row_list.length) {
                    ...
                    return DataRow(
                      cells: [
                        ...
                        DataCell(Text(
                            industries[one_row_list[index].industry])), // 수정
